...
export const CustomerLogoMarquee: React.FC<CustomerLogoMarqueeProps> = ({
  ...
  return (
    <section aria-label="Customer logos" className={cn("py-10 md:py-12 bg-ink-2/50 border-y border-white/[0.08] overflow-hidden relative", className)}>
      ...
      <div className="max-w-site mx-auto px-4 sm:px-6 lg:px-8 mb-6 text-center relative z-20">
        <p className="text-xs sm:text-sm font-display font-semibold tracking-widest text-text-dim uppercase">
          {title}
        </p>
      </div>
            ...
            {row1Logos.map((logoUrl, index) => {
              ...
              return (
                <div
                  ...
                  className="inline-flex items-center justify-center shrink-0 w-40 sm:w-48 md:w-52 h-18 sm:h-20 px-4 bg-white/[0.04] hover:bg-white/[0.09] rounded-xl border border-white/[0.08] transition-all duration-300 group shadow-sm hover:border-indigo/40 hover:shadow-glow-indigo"
                >
                  <img
                    src={logoUrl}
                    alt={`iEYAL Customer - ${cleanName}`}
                    className="max-h-12 sm:max-h-14 md:max-h-16 max-w-[85%] object-contain transition-transform duration-300 group-hover:scale-110"
                    loading="lazy"
                  />
                </div>
              );
